...
def CRF_Map_Cython(float[:, :, :] img, float[:] I, float[:] B):
    ...
    cdef float tiny_bin = 9.7656e-04      # 1/1024 = 9.7656e-04
    cdef float min_tiny_bin = 0.0039
...
def ICRF_Map_Cython(float[:, :, :] img, float[:] invI, float[:] invB):
    cdef Py_ssize_t h = img.shape[0]
    cdef Py_ssize_t w = img.shape[1]
    cdef Py_ssize_t c = img.shape[2]
